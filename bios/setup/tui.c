...
static void write_char(uint8_t ch, uint8_t attr) {
    bios_video_write_char_attr(ch, 0, attr, 1);
}

uint8_t tui_set_xpos(uint8_t x, uint8_t len, uint8_t align) {
    if (align == ALIGN_CENTER) {
        return (uint8_t)(x - len / 2);
    } else if (align == ALIGN_RIGHT) {
        return (uint8_t)(x - len);
    }
    return x;
}

void tui_char_at(uint8_t ch, uint8_t x, uint8_t y, uint8_t attr) {
    set_cursor_pos(x, y);
    write_char(ch, attr);
}
    ...
    uint8_t selected, uint8_t attr, uint8_t hi_attr, const char* const items[])
{
    ...
    for (uint8_t i = 0; i < count; ++i) {
        ...
        xpos += space;
        if (i == selected) {
            tui_char_at(' ', xpos - 1, y, item_attr);
            tui_text(xpos, ypos, items[i], item_attr);
            tui_char_at(' ', xpos + len, y, item_attr);
        } else {
            tui_text(xpos, ypos, items[i], item_attr);
        }
        xpos += len;
...
void tui_border(uint8_t x, uint8_t y, uint8_t w, uint8_t h,
    uint8_t fg_attr, uint8_t bg_attr, uint8_t border, uint8_t align)
{
    uint8_t xpos = tui_set_xpos(x, w, align);
    uint8_t ypos = y;
    uint8_t attr = (bg_attr << 4) | fg_attr;
    ...
    tui_char_at(top_left, xpos, ypos, attr);
    tui_char_at(top_right, xpos + w - 1, ypos, attr);
    tui_char_at(bottom_left, xpos, ypos + h - 1, attr);
    tui_char_at(bottom_right, xpos + w - 1, ypos + h - 1, attr);

    for (uint8_t i = 1; i < w - 1; ++i) {
        tui_char_at(horizontal, xpos + i, ypos, attr);
        tui_char_at(horizontal, xpos + i, ypos + h - 1, attr);
    }
    for (uint8_t i = 1; i < h - 1; ++i) {
        tui_char_at(vertical, xpos, ypos + i, attr);
        tui_char_at(vertical, xpos + w - 1, ypos + i, attr);
    }
}

void tui_fill(uint8_t x, uint8_t y, uint8_t w, uint8_t h, uint8_t ch, uint8_t fg, uint8_t bg)
{
    bios_video_scroll_wnd_down(h, bg << 4 | fg, x, x + w - 1, y, y + h - 1);
}
    ...
    uint8_t count, const char* const items[])
{
    ...
    for (uint8_t i = 0; i < count; ++i)
    {
        tui_text(xpos, ypos, items[i], (i == selected) ? (hi_attr) : (attr));
        ypos++;
    }
}
...
void tui_horizontal_line(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, uint8_t attr)
{
    for (uint8_t i = x0; i < x1; ++i)
        tui_char_at(0xC4, i, y0, attr);

    set_cursor_pos(0xFF, 0xFF);
}

void tui_text_item(uint8_t x, uint8_t y, uint16_t len,
    const char __far* item, const char __far* value,
    uint8_t attr, uint8_t hi_attr, bool selected)
{
    uint8_t xpos = x;
    uint8_t ypos = y;

    tui_text(xpos, ypos, item, attr);
    xpos += len;

    if (selected) {
        tui_text(xpos, ypos, value, hi_attr);
    } else {
        tui_text(xpos, ypos, value, attr);
    }
}
    ...
    uint8_t count, const char* const items[])
{
    ...
    for (uint8_t i = 0; i < count; ++i) {
        if (i == selected) {
            tui_text(xpos, ypos, items[i], hi_attr);
        } else {
            tui_text(xpos, ypos, items[i], attr);
        }
        ypos++;
    }
}
